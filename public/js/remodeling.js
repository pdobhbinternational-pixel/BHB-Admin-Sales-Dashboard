...
function getRadioValue(name, otherInputId = null) {
  ...
  // Handle custom "Other" text input (including "Other Florida Location")
  if ((selected.value === 'Other' || selected.value.startsWith('Other Florida Location')) && otherInputId) {
    const customText = document.getElementById(otherInputId)?.value.trim();
    if (customText) {
      return `${selected.value}: ${customText}`;
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', (e) => {
      ...
      const summaryData = [
        ...
        { label: 'Property Address', value: document.getElementById('address').value },
        { label: 'Property Location', value: getRadioValue('propertyLocation', 'locationOther') },
        { label: 'Property Type', value: getRadioValue('propertyType', 'propertyTypeOther') },
        { label: 'Main Focus Area', value: getRadioValue('focusArea') },
        { label: 'Material & Style Preference', value: getRadioValue('materialStyle', 'materialOther') },
        { label: 'Target Start Date', value: getRadioValue('startDate', 'startDateOther') },
        { label: 'Target Investment', value: getRadioValue('targetInvestment', 'investmentOther') },
        { label: 'Design Dreams', value: document.getElementById('designDreams').value },
        { label: 'How They Found Us', value: getRadioValue('leadSource', 'leadSourceOther') }
      ];

      summaryData.forEach(item => {
        if (!item.value || item.value === 'Not specified') return;
  ...
  if (finalSubmitBtn) {
    finalSubmitBtn.addEventListener('click', async () => {
      ...
      const payload = {
        business_slug: document.getElementById('businessSlug')?.value || 'remodeling',
        ...
        propertyLocation: getRadioValue('propertyLocation', 'locationOther'),
        propertyType: getRadioValue('propertyType', 'propertyTypeOther'),
        focusArea: getRadioValue('focusArea'),
        materialStyle: getRadioValue('materialStyle', 'materialOther'),
        startDate: getRadioValue('startDate', 'startDateOther'),
        targetInvestment: getRadioValue('targetInvestment', 'investmentOther'),
        designDreams: document.getElementById('designDreams').value,
      ...
      mainContainer.innerHTML = `
        ...
        ">
          ...
          <p style="font-size: 1.25rem; color: #4b5563; max-width: 600px; line-height: 1.6;">
            Your remodeling inquiry has been submitted successfully, and we have received your request and will reach out to you shortly, so please check your emails regularly.
          </p>
        </div>
      `;
